...
class SkillPlanMilestoneRequest(BaseModel):
    ...
    reward_xp: int = Field(default=0, ge=0, le=100_000)
...
class SkillPlanCreateRequest(BaseModel):
    ...
    badge_icon: str = Field(default="🏆", min_length=1, max_length=20)
    badge_colour: str = Field(default="cyan", min_length=2, max_length=30)
...
def _skill_plan_payload(
    ...
    return {
        ...
        "badge_icon": tree.badge_icon,
        "badge_colour": tree.badge_colour,
        "active": tree.active,
...
def create_skill_plan(
    ...
    programme = get_programme(db)
    if bool(data.player_id) == bool(data.group_id):
        raise HTTPException(status_code=400, detail="Choose exactly one assignment target: young person or group.")

    if data.player_id is not None:
        players = db.query(Player).filter(Player.id == data.player_id, Player.programme_id == programme.id, Player.active == True).all()
    else:
        group = db.query(YouthGroup).filter(YouthGroup.id == data.group_id, YouthGroup.programme_id == programme.id, YouthGroup.active == True).first()
        if group is None:
            raise HTTPException(status_code=404, detail="Group not found or inactive.")
        players = db.query(Player).filter(Player.group_id == group.id, Player.programme_id == programme.id, Player.active == True).all()

    if not players:
        raise HTTPException(status_code=404, detail="No active young people found for this assignment.")
    ...
    milestone_xp = [m.required_xp for m in data.milestones]
    if milestone_xp != sorted(milestone_xp) or len(set(milestone_xp)) != len(milestone_xp):
        raise HTTPException(status_code=400, detail="Skill Badge stage XP thresholds must increase and be different.")

    created=[]; skipped=[]
    for player in players:
        active_tree = db.query(SkillTree).filter(SkillTree.player_id == player.id, SkillTree.active == True, SkillTree.completed == False).first()
        if active_tree is not None:
            skipped.append({"player_id":player.id,"gamertag":player.gamertag,"reason":"active_skill_badge_exists"})
            continue
        previous_trees = db.query(SkillTree).filter(SkillTree.player_id == player.id).all()
        tree = SkillTree(player_id=player.id,name=data.name.strip(),description=data.description.strip() if data.description else None,badge_icon=data.badge_icon,badge_colour=data.badge_colour,active=True,completed=False,current_xp=0,tree_number=max((x.tree_number for x in previous_trees),default=0)+1)
        db.add(tree); db.flush()
        for sort_order,milestone in enumerate(data.milestones,1):
            db.add(SkillMilestone(skill_tree_id=tree.id,name=milestone.name.strip(),required_xp=milestone.required_xp,sort_order=sort_order,reward_description=milestone.reward_description.strip() if milestone.reward_description else None,reward_xp=milestone.reward_xp,completed=False))
        created.append(tree)

    if not created:
        raise HTTPException(status_code=409, detail="Every selected young person already has an active Skill Badge.")

    audit(db,user.id,"skill_badge.created",f"name={data.name.strip()};created={len(created)};skipped={len(skipped)}")
    db.commit()
    for tree in created: db.refresh(tree)
    return {"created":[_skill_plan_payload(tree,tree.player.gamertag) for tree in created],"skipped":skipped}


@router.post("/skill-plans/{skill_tree_id}/milestones/{milestone_id}/complete")
def complete_skill_badge_stage(skill_tree_id:int,milestone_id:int,user=Depends(require_roles("admin","youth_worker")),db:Session=Depends(get_db)):
    programme=get_programme(db)
    tree=db.query(SkillTree).join(Player,SkillTree.player_id==Player.id).filter(SkillTree.id==skill_tree_id,Player.programme_id==programme.id,Player.active==True).first()
    if tree is None: raise HTTPException(status_code=404,detail="Skill Badge not found.")
    milestone=db.query(SkillMilestone).filter(SkillMilestone.id==milestone_id,SkillMilestone.skill_tree_id==tree.id).first()
    if milestone is None: raise HTTPException(status_code=404,detail="Skill Badge stage not found.")
    if milestone.completed: raise HTTPException(status_code=409,detail="This stage is already complete.")
    previous=db.query(SkillMilestone).filter(SkillMilestone.skill_tree_id==tree.id,SkillMilestone.sort_order<milestone.sort_order).order_by(SkillMilestone.sort_order.desc()).first()
    if previous and not previous.completed: raise HTTPException(status_code=409,detail="Complete the previous Skill Badge stage first.")
    milestone.completed=True; milestone.completed_at=datetime.utcnow()
    awarded=int(milestone.reward_xp or 0)
    tx=None
    if awarded:
        tx=award_xp(db,programme_id=tree.player.programme_id,player_id=tree.player_id,amount=awarded,group_amount=0,transaction_type="skill_badge_stage",reason=f"Skill Badge: {tree.name} / Stage: {milestone.name}",reference_type="skill_badge_stage",reference_id=milestone.id,idempotency_key=f"skill-badge-stage:{milestone.id}",created_by=user.id)
    stages=db.query(SkillMilestone).filter(SkillMilestone.skill_tree_id==tree.id).order_by(SkillMilestone.sort_order).all()
    if stages and all(x.completed for x in stages): tree.completed=True; tree.completed_at=datetime.utcnow(); tree.active=False
    audit(db,user.id,"skill_badge.stage_completed",f"skill_tree_id={tree.id};milestone_id={milestone.id};reward_xp={awarded};transaction_id={tx.id if tx else None}")
    db.commit(); db.refresh(tree)
    return _skill_plan_payload(tree,tree.player.gamertag)
