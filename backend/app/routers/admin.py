...
def add_player_to_staff_group(
    ...
    # A player belongs to one group at a time. Never silently move
    # them between groups: staff must explicitly confirm the move.
    if previous_group_id is not None and previous_group_id != group.id:
        previous_group = db.get(YouthGroup, previous_group_id)
        previous_name = (
            previous_group.name
            if previous_group is not None
            else f"Group #{previous_group_id}"
        )
        ...
        if not confirm_move:
            raise HTTPException(
                status_code=409,
                detail=(
                    f"This young person is already in "
                    f'"{previous_name}". '
                    f'Confirm the move to "{group.name}" before continuing.'
                ),
            )
