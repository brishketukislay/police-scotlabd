    ...
    *,
    ...
    return {
        ...
        **({
            "segments": [
                {
                    "label": f"{int(value):,} XP",
                    "value": int(value),
                }
                for value in (game.prize_values or [])
            ]
        } if not include_prizes else {}),
...
def player_reward_games(
    ...
    for game in games:
        ...
        payload = {
            **serialize_game(game, now, include_prizes=False),
...
def play_reward_game(
    ...
    # A reward game must have at least one configured reward.
    # Do this check before consuming the entitlement or awarding anything.
    if not game.prize_values:
        raise HTTPException(
            status_code=409,
            detail="No rewards are configured for this game.",
        )

    prizes = validate_prizes(game.prize_values)

    # validate_prizes may reject an empty/invalid configuration as well,
    # but keep the explicit guard above so the player gets a clear message.
    if not prizes:
        raise HTTPException(
            status_code=409,
            detail="No rewards are configured for this game.",
...
@router.post("/admin")
def create_reward_game(data:RewardGameCreateRequest,user=Depends(require_roles("admin","youth_worker")),db:Session=Depends(get_db)):
...
def update_reward_game(game_id:int,data:RewardGameUpdateRequest,user=Depends(require_roles("admin","youth_worker")),db:Session=Depends(get_db)):
